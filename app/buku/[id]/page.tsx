...
export default async function BookDetail({ params }: { params: { id: string } }) {
  ...
  if (session) {
    const { data: bm } = await supabase.from('bookmarks').select('id').eq('user_id', session.userId).eq('book_id', b.id).maybeSingle()
    bookmarked = !!bm
  }

  const user = session
    ? { name: session.profile.full_name || session.profile.username || 'Pengguna', isStaff: staff }
    : null
        ...
        <div className="card">
          <div className="flex flex-col gap-4 sm:flex-row">
            {b.cover_url ? (
              ...
              <img src={b.cover_url} alt="" className="mx-auto h-52 w-36 rounded-2xl object-cover shadow-md sm:mx-0" />
            ) : (
              <div className="mx-auto grid h-52 w-36 place-items-center rounded-2xl bg-amber-100 text-5xl sm:mx-0">📘</div>
            )}
            <div className="flex-1">
              <h1 className="font-display text-xl font-bold text-slate-800">{b.judul_buku}</h1>
              <p className="mt-1 text-sm text-slate-500">{b.pengarang}</p>
              {b.deskripsi && <p className="mt-3 text-sm leading-relaxed text-slate-600">{b.deskripsi}</p>}
              <div className="mt-4 no-print">
                <BookmarkButton bookId={b.id} initial={bookmarked} isLoggedIn={!!session} />
              </div>
            </div>
          </div>

          <dl className="mt-5 grid grid-cols-1 gap-x-6 gap-y-1 sm:grid-cols-2">
            {FIELDS.map((f) => (
              <div key={f.key} className="flex justify-between gap-3 border-b border-slate-100 py-1.5 text-sm">
                <dt className="text-slate-500">{f.label}</dt>
                <dd className="text-right font-bold text-slate-800">{(b as any)[f.key] || '-'}</dd>
              </div>
            ))}
          </dl>
